...
function tmux(args: string[], socket = SOCKET): { ok: boolean; stdout: string; stderr: string } {
...
function ctl(args: string[], home: string, socket = SOCKET): { exitCode: number; stdout: string; stderr: string } {
...
function reportAndReadBackViaStatus(reason: string): { home: string; report: unknown } {
  ...
  const name = `report-nl-${suffix}`;
  const socket = `${SOCKET}-${suffix}`;
  try {
    const created = tmux(["new-session", "-d", "-s", name, "-c", "/tmp"], socket);
    expect(created.ok).toBe(true);

    const reportResult = ctl(["session", "report", "--target", name, "--outcome", "blocked", "--reason", reason], home, socket);
    expect({ exitCode: reportResult.exitCode, stderr: reportResult.stderr }).toMatchObject({ exitCode: 0 });

    const statusResult = ctl(["status"], home, socket);
    expect({ exitCode: statusResult.exitCode, stderr: statusResult.stderr }).toMatchObject({ exitCode: 0 });
    const statusParsed = JSON.parse(statusResult.stdout);
    const session = statusParsed.sessions.find((s: { name: string }) => s.name === name);
    expect(session).toBeDefined();
    ...
    tmux(["kill-session", "-t", name], socket);
    tmux(["kill-server"], socket);
