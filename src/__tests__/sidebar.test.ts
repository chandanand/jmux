...
describe("Sidebar", () => {
  ...
  test("the last row belongs to the session viewport", () => {
    const sidebar = new Sidebar(SIDEBAR_WIDTH, 6);
    sidebar.updateSessions(makeSessions([{ name: "main" }]));
    sidebar.getGrid();

    // Header rows 0-1, Command Center + spacer at 2-3, then the session's
    // name and detail rows at 4-5. The bottom row is content, not a footer.
    expect(sidebar.getSessionByRow(5)?.name).toBe("main");
  });
