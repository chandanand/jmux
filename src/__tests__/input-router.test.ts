...
describe("Command Center with the panel docked", () => {
  ...
  test("Ctrl-Space g toggles the panel from the glass arm and leaks nothing", () => {
    const sent: string[] = [];
    let toggled = 0;
    const router = new InputRouter({
      onPtyData: (d) => sent.push(d),
  ...
  test("Ctrl-Space z zooms the panel while it holds focus, the tile otherwise", () => {
    let panelZoom = 0;
    let tileZoom = 0;
    const router = new InputRouter({
      onPtyData: () => {},
    ...
    router.handleInput("\x00"); router.handleInput("z");
