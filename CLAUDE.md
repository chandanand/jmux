...
- **A refusal belongs on the page that caused it.** Rejections were reported through `showToast`, which is consumed only by the *toolbar's* status chip — the far end of the screen from a centred modal, and transient. A directory that did not exist was refused, correctly, somewhere the user was not looking, and it read as the flow having hung. Ports return `{ ok, message }`; the modal draws it above the action bar.
- **A tick means somebody chose, and every step must be answerable.** Two related failures, both from treating an absent value as an answer: `presetForCommand(undefined)` is `off`, so the naming page ticked "leave sessions unnamed" while the map reported the same step as `not yet`; and choosing "unnamed" stored *no* command, which is indistinguishable from never having answered, so the step stayed outstanding and the toolbar dot nagged about a preference already expressed. `setup.sessionTitle` records the declaration, the same tier and reason as `setup.tracker` and `setup.agentHooks` beside it.
- **A placeholder is not a default.** Hint text rendered where the value goes reads as a filled field, so Enter looks like it should commit — and `InputModal` silently consumes Enter on an empty buffer, so the flow looks hung. Collectors open on a real editable value, and `InputModal.requiredHint` makes an empty commit say why instead of eating the key.
...
Session creation is deliberately **not** in the flow. `NewSessionModal` owns
the full provisioning path, including the configured agent launch, and failed
provisioning leaves debris by design, so the finish page hands off to it rather
than growing a second implementation. `onboarding-integration.test.ts` drives
a real pty and asserts no installer output reaches the frame — the regression
that caused the rebuild is invisible to every unit test either side of it.
...
**Session cleanup shares one destructive preflight (`cli/session-cleanup.ts`) between the CLI and command palette.** It resolves a pane subdirectory to the linked worktree root, refuses primary checkouts, and inspects tracked and untracked changes before tmux is touched. The CLI cannot clean its own session because killing it would kill the process before worktree removal; the TUI can, because its process owns the tmux client from outside the session. In the palette a dirty preflight changes the confirmation into an explicit discard action, and a clean confirmation is preflighted again without force immediately before the kill — a file written while the modal is open stops cleanup rather than being destroyed under stale consent. The branch is never deleted.
...
- **The toggle is inert on a running server, so it is disclosed twice.** `-f` is read only when tmux *starts* a server, and a settings change moves no asset hash — so `@jmux-config-generation` carries a second half (`<assetHash>.<confTag>`) and `staleGenerationNotice` names *which* half diverged, because "jmux was upgraded" is not something a user who just edited `userTmuxConfig` can act on. A stale stamp remains stale until the server is genuinely repaired; overwriting it after displaying one warning makes every later attach falsely quiet. The settings row adds `getNote` → `restart to apply`, compared against what this process resolved at boot rather than relying on that server stamp. Demo mode forces the setting off in `setupDemo`, because it starts its own server before `main.ts` has resolved anything.
...
- **`-f <config>` is honored only when tmux *starts* a server.** Attaching to a running server ignores it, and jmux deliberately never `source-file`s over the control channel. An upgraded jmux therefore keeps the old config until the server dies — `config-generation.ts` preserves a stale `@jmux-config-generation` stamp and reports the mismatch rather than letting it be silent. The stamp is not sufficient by itself: every option in `core.conf` is also read back before an unstamped server earns one, because a user config can block halfway through loading while the control client still attaches.
